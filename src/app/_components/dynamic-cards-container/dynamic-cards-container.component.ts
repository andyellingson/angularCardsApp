...
@Component({
  ...
  styleUrls: ['./dynamic-cards-container.component.less'],
...
export class DynamicCardsContainerComponent implements OnInit {
  ...
  DealersTurn(){
    ...
    EndDealerTurn(){
      localStorage.setItem('bank', this.Bank.toString());
  ...
  GetBankAmount():number{
    var currentAmount = +localStorage.getItem('bank');
    if(currentAmount <= 0)
    {
